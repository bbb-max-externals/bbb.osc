...
class bbb_osc_path_head : public object<bbb_osc_path_head> {
public:
    MIN_DESCRIPTION{"Keep the first N components of an OSC path"};
    MIN_TAGS{"osc, path"};
    MIN_AUTHOR{"ISHII 2bit"};

    inlet<> input{this, "(anything) /a/b/c payload..."};
    outlet<> output{this, "(anything) /a/b payload..."};
    ...
    message<> anything_msg{this, "anything", "Keep head components",
        MIN_FUNCTION {
            if(args.size() < 1 || args[0].a_type != c74::max::A_SYM) return {};

            std::string path(static_cast<const char*>(symbol(args[0])));
            std::string result_path = bbb::osc::path::head(path, static_cast<size_t>(std::max(0, static_cast<int>(count))));
...
MIN_EXTERNAL(bbb_osc_path_head);
